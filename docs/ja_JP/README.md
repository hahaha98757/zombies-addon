...
- Server モードは Zombies Utils v1.4.4に基づいています。サーバーの速度をすぐに反映して時間を測定します。
...
- テキストスタイルは、デフォルトの「W1：0.10.0」、「W1: 0.10.00」、「W1 00:10」です。
...
- ホットキー（デフォルトUp）でRL Modeを切り替えます。ウェーブディレイが設定されたティックだけ追加されて表示されます。
- ホットキー（デフォルトUp）でWrath Modeを切り替えます。ウェーブディレイが設定されたティックだけ追加されて表示されます。
...
- RL Modeオフセット：RLモードでウェイヴ遅延にオフセットを追加します。
- Wrath Modeオフセット：Wrath Modeでウェイヴ遅延にオフセットを追加します。
...
- 言語：このmodの言語
- スピードランモード：スピードランモードを有効または無効にします。speedrun.comルールで禁止されている機能の一部を無効にします。
