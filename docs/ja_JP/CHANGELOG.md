...
2026-05-03
- 変更
  - 互換性のある Zombies Utils バージョン：1.3.7 -> 1.4.2
  - Internal Timer: Server モードが倒れ、死亡状態でも維持される。
- 修正：Better Zombies Left
  - 残りのゾンビラインをスコアではなく文字列を含むかどうかを検出します。
  - 4.7.2にロールバック。
- バグ修正
  - Powerup Patterns：タイマーが0以下に進む。
  - すでに存在するゲームのサーバー番号で新しいゲームを開始したい場合は、既存のゲームを削除します。

## 4.7.3
2026-04-26
- 変更：Better Zombies Left
  - Feather Clientなどのスコアボード関連モードでも動作します。
- バグ修正：Internal Timer
  - サーバーモードでゲームオーバー時に常にクライアントモードを使用するバグを修正しました。
- コード修正：WaveDelays.playSound() 呼び出し条件

## 4.7.2
2026-04-10
- 修正：翻訳文字列
  - 自動更新のウィンドウのみのテキストを削除します。

## 4.7.1
2026-04-10
- 変更：自動更新は他のOSで利用可能です。
- コード修正：意味のない super.initGui() 削除。
...
- 追加：PRRモブスポーンデータ。
- 変更：Recorder
  - タイマー補正失敗時のラウンド1区間記録、全記録を測定しない。

## 4.7.0-beta2
2026-02-26
- バグ修正：Internal Timer
  - ClientモードでWave Delaysのサウンド再生が2回再生されるバグ。
  - ゲーム中にモードが変更されたときにタイマーが無効な値を持つバグ。

## 4.7.0-beta1
2026-02-25
- 修正：Internal Timer
  - コンフィグオプションの追加：タイマー計算の基準をクライアントとサーバーの間で選択できるように修正。
    - クライアント：従来の方法。クライアントTPSに基づいてタイマーを計算します。サーバー基準20ティックごとにサーバー時間と同期。
    - サーバー：サーバーTPSに基づいてタイマーを計算します。倒れ、死亡、再接続時にタイマーが停止し、クライアントタイマーとマージして補正。
  - タイマー補正失敗時以降補正をしないように修正。
...
- 追加：Better Zombies Left
  - DER、PRNモブスポーン情報を追加。
  - PRH、PRRヘリコプター脱出モブスポーン情報を追加。
- 修正
  - 間違ったバージョンの場合、更新を確認しないように修正。
  - シングルプレイで動作しないように修正：デバッグモードを有効にする必要があります。
- コード修正
  - modFileオブジェクトの初期化方法。
  - Better Zombies Leftの仕組み。
