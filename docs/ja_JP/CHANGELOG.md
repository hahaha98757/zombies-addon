...
- コード修正
  - 一部をすぐにアクセスできるように修正
  - モジュールのクリーンアップといくつかのクラスをオブジェクトに変更
  - コンフィグコードの修正

## 4.8.4
2026-06-21
- コード修正
  - Gradle アップデート ロールバック：8.14.4 -> 8.8

## 4.8.3
2026-06-02
- 追加
    - スピードランモード
    - Internal Timer: テキストスタイルオプション
- アンインストール: blockUnlegitMods, detectUnlegitMods オプション
- 変更: Wave Delays
    - テキストスタイルオプション
    - Wrath Modeを追加。
- バグ修正
    - ゲーム開始時に RoundStartEvent が発生しない。
    - ゲーム開始時にすぐにゲームが削除される。
- コード修正
    - Kotlinアップデート：2.2.20 - > 2.3.21
    - Gradleアップデート：8.8 - > 8.14.4

## 4.8.1
2026-05-06
- バグ修正：タイマー補正
  - 練習サーバーでタイマー補正がキャンセルされない。

## 4.8.1
2026-05-06
- 変更：Zombies Utilsの設定
  - 互換性のある推奨バージョン：1.4.2 -> 1.4.4

## 4.8.0
2026-05-05
- 修正：Last Weapons
  - アラビア数字のレベルを認識しない：練習サーバーの修正を反映。
...
- 変更：練習サーバーの互換性
  - レコーダーが記録を保存しない。
  - 難易度検知を昔ながら使用。
  - Last Weaponsはアラビア数字のレベルを認識します。
  - 情報のないラウンドでモジュールが機能しない。
  - ロビーからスコアボード情報を削除しました。
